...
export const NewPostPage: React.FC = () => {
  ...
  const fetchLocationSuggestions = async () => {
    try {
      const response = await axiosInstance.get('http://127.0.0.1:8000/api/platform/locations/');
  ...
  const fetchHashtagSuggestions = async () => {
    try {
      const response = await axiosInstance.get('http://127.0.0.1:8000/api/platform/hashtags/');
  ...
  const handleCreateLocation = async () => {
    if (newLocationCountry && newLocationCity) {
      try {
        const response = await axiosInstance.post('http://127.0.0.1:8000/api/platform/locations/', {
  ...
  const handleCreateHashtag = async () => {
    if (newHashtagName) {
      try {
        const response = await axiosInstance.post('http://127.0.0.1:8000/api/platform/hashtags/', {
          name: newHashtagName,
        });
