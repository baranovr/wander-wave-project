...
export const login = createAsyncThunk(
  ...
  async (credentials: { email: string; password: string }, { dispatch }) => {
    try {
      const response = await axiosInstance.post('http://127.0.0.1:8000/api/user/token/', credentials);
...
export const refreshToken = createAsyncThunk(
  ...
  async (_, { rejectWithValue }) => {
    try {
      ...
      const response = await axiosInstance.post('http://127.0.0.1:8000/api/user/token/refresh/', { refresh: refreshToken });
