...
export const fetchUserProfile = createAsyncThunk(
  ...
  async (_, { dispatch, rejectWithValue }) => {
    try {
      const response = await retryWithBackoff(() => axiosInstance.get('http://127.0.0.1:8000/api/user/my_profile/'));
      ...
      if (error.response && error.response.status === 401) {
        ...
        try {
          ...
          const retryResponse = await axiosInstance.get('http://127.0.0.1:8000/api/user/my_profile/');
...
export const fetchMyLiked = createAsyncThunk(
  ...
  async (_, { rejectWithValue }) => {
    try {
      const response = await axiosInstance.get('http://127.0.0.1:8000/api/user/my_profile/my_liked/');
      return response.data;
    } catch (error) {
...
export const fetchMyFavorites = createAsyncThunk(
  ...
  async (_, { rejectWithValue }) => {
    try {
      const response = await axiosInstance.get('http://127.0.0.1:8000/api/user/my_profile/my_favorites/');
      return response.data;
    } catch (error) {
...
export const updateUserProfile = createAsyncThunk(
  ...
  async (userData: Partial<User>, { dispatch, rejectWithValue }) => {
    try {
      const response = await retryWithBackoff(() => axiosInstance.patch('http://127.0.0.1:8000/api/user/my_profile/', userData));
      return response.data;
    } catch (error: any) {
      if (error.response && error.response.status === 401) {
        try {
          await dispatch(refreshToken()).unwrap();
          const retryResponse = await axiosInstance.patch('http://127.0.0.1:8000/api/user/my_profile/', userData);
          return retryResponse.data;
        } catch (refreshError) {
          return rejectWithValue('Session expired. Please log in again.');
        }
