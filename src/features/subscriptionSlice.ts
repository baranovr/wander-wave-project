  ...
  async ({ postId }: { postId: number }, { rejectWithValue }) => {
    try {
      const response = await axiosInstance.post(
        `https://wander-wave-backend.onrender.com/api/platform/posts/${postId}/author-profile/subscribe/`,
      );
  ...
  async ({ postId }: { postId: number }, { rejectWithValue }) => {
    try {
      const response = await axiosInstance.delete(
        `https://wander-wave-backend.onrender.com/api/platform/posts/${postId}/author-profile/unsubscribe/`,
      );
      return response.data;
    } catch (error) {
